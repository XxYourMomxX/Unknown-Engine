...
class Paths
{
	...
	{
		return getModPath('data/$key.json', TEXT, library);
	}
	...
	inline static public function getModSparrowAtlas(key:String, ?library:String)
	{
		return FlxAtlasFrames.fromSparrow(modImages(key, library), file('images/$key.xml', library));
	}
